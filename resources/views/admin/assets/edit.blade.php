...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="installated_at">{{ trans('cruds.asset.fields.installated_at') }}</label>
                <input class="form-control date {{ $errors->has('installated_at') ? 'is-invalid' : '' }}" type="text" name="installated_at" id="installated_at" value="{{ old('installated_at', $asset->installated_at) }}">
                @if($errors->has('installated_at'))
                    <span class="text-danger">{{ $errors->first('installated_at') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.installated_at_helper') }}</span>
            </div>
            <div class="form-group">
